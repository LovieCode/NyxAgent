...
| 主导航 | `pages/files/files` | 根目录、面包屑、搜索、排序、导入、新建文件/目录、打开、重命名、删除 | 测试中 | V5 首轮布局与搜索/排序已通过 27 页 Android 差量编译；导入待 ADB 实测 |
| 主导航 | `pages/settings/settings` | 所有设置入口、资料摘要、状态展示 | 测试中 | V5 首轮布局已通过 27 页 Android 差量编译，待 ADB 逐入口回归 |
| 设置 | `pages/settings/diagnostics/diagnostics` | 错误记录、诊断编号、技术详情、清空、空状态 | 测试中 | 已注册为第 28 个页面并通过 Android class 全量编译；ADB 入口与异常记录回归待完成 |
...
| 13 | 残留工作区显示名 | 名称快照与中央元数据保留最后名称；旧内置目录恢复默认名称 | UTS 静态扫描、HBuilderX 5.15 Android class 编译通过 | 待提交 |
| 14 | 长对话渲染性能 | 私聊/群聊固定大小双向滑动窗口；上下均批量加载和卸载；群流式事件合并刷新 | UTS 静态扫描、HBuilderX 5.15 Android class 编译通过；模拟器已关闭，滚动手感待回归 | 待提交 |
| 15 | 群聊成员私发 | 用户显式请求时锁定 `user_private`；其他成员、调度与复盘只读公开投影；原成员保留自己的私发历史 | HBuilderX 5.15 Android class 全量编译通过；ADB 消息投递待回归 | 待提交 |
| 16 | 统一错误链 | 结构化诊断、LLM transport error、失败气泡、设置事务与文件操作错误提示 | HBuilderX 5.15 Android class 全量及多轮热编译通过；ADB 冷启动日志通过 | 待提交 |
